...
def process_shortcodes(content, shared_dir=None, includes_list=None):
    ...
    content = NOTE_SHORTCODE_PATTERN.sub(convert_note_shortcode, content)
...
def convert_file(input_path, output_path, shared_dir=None):
    ...
    try:
        ...
        if frontmatter_match:
            ...
            main_content = process_shortcodes(main_content, shared_dir, includes_list)
    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            if file.endswith('.md'):
                ...
                # Convert the file with access to shared directory
                if convert_file(input_path, output_path, shared_dir):
                    count_success += 1
