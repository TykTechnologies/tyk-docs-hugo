#!/usr/bin/env python3
"""
Build canonical_map.json for v3.0-LTS by resolving each sitemap URL's
...
Map key:   relative path after /docs/3-lts/  e.g. "getting-started/key-concepts/"
...
SITEMAP_URL    = "https://tyk.io/docs/3-lts/sitemap.xml"
V_PREFIX       = "https://tyk.io/docs/3-lts/"  # strip this to get the map key
...
SITEMAP_PATH   = Path("/tmp/hugo-out-v3-lts/sitemap.xml")  # local build output
HUGO_OUT_DIR   = Path("/tmp/hugo-out-v3-lts")              # Hugo output directory
MAX_WORKERS    = 15
...
def fetch_sitemap_urls() -> list[str]:
    """Build URL list from local Hugo output directory (sitemap is empty on this branch)."""
    urls = []
    for index_html in sorted(HUGO_OUT_DIR.rglob("index.html")):
        # Get relative path from hugo output dir
        rel = index_html.parent.relative_to(HUGO_OUT_DIR)
        rel_str = str(rel)
        if rel_str == ".":
            # Root index — skip, it's the home page
            continue
...
def resolve_url(
    ...
    map_key  = path after /docs/3-lts/   e.g. "getting-started/key-concepts/"
...
def main():
    ...
    print(f"\nBuilding URL list from local Hugo output: {HUGO_OUT_DIR}")
