#!/usr/bin/env python3
"""
Build canonical_map.json for v3.1 by resolving each sitemap URL's
...
Map key:   relative path after /docs/3.1/  e.g. "getting-started/key-concepts/"
...
SITEMAP_URL    = "https://tyk.io/docs/3.1/sitemap.xml"
V_PREFIX       = "https://tyk.io/docs/3.1/"  # strip this to get the map key
...
HUGO_OUT_PATH  = Path("/tmp/hugo-out-v3.1")  # local Hugo build output directory
MAX_WORKERS    = 15
REQUEST_TIMEOUT = 15  # seconds per URL
HEADERS        = {"User-Agent": "Mozilla/5.0 (compatible; tyk-canonical-mapper/1.0)"}

# Aliases for compatibility with shared logic below
V55_PREFIX = V_PREFIX
OLD_BASE   = CANONICAL_BASE


def fetch_sitemap_urls() -> list[str]:
    """Build URL list from local Hugo output by enumerating index.html files.

    The sitemap.xml for v3.1 was intentionally emptied as part of the noindex
    rollout, so we derive URLs directly from the build output directory tree.
    """
    urls = []
    for p in sorted(HUGO_OUT_PATH.rglob("index.html")):
        rel = p.parent.relative_to(HUGO_OUT_PATH)
        rel_str = str(rel)
        if rel_str == ".":
            url = V_PREFIX
        else:
...
def main():
    ...
    print(f"Found {len(urls)} URLs from local build output")
