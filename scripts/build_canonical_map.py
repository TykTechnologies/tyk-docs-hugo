#!/usr/bin/env python3
"""
Build canonical_map.json for v4.0 by resolving each sitemap URL's
...
Map key:   relative path after /docs/4.0/  e.g. "getting-started/key-concepts/"
...
SITEMAP_URL    = "https://tyk.io/docs/4.0/sitemap.xml"
SITEMAP_LOCAL  = Path("/tmp/hugo-out-v4.0/sitemap.xml")  # local fallback if live is blocked
V55_PREFIX     = "https://tyk.io/docs/4.0/"  # strip this to get the map key
...
def fetch_sitemap_urls(sitemap_url: str) -> list[str]:
    # Try local file first (in case live site is behind Cloudflare challenge)
    if SITEMAP_LOCAL.exists():
        print(f"  Using local sitemap: {SITEMAP_LOCAL}", file=sys.stderr)
        xml = SITEMAP_LOCAL.read_bytes()
    else:
...
def resolve_url(
    ...
    map_key  = path after /docs/4.0/   e.g. "getting-started/key-concepts/"
