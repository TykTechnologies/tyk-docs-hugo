...
You do not need to define your whole API in the editor, only those paths you want to manage. The exception to this is if you are using an Allowlist, in which case you will need to specify every endpoint as all others will be blocked.
{{< /note >}}


By default, importing an API using Swagger/OpenAPI or API Blueprint JSON definitions will generate an Allowlist.
...
When using Regular Expressions with the following plugins (Mock Response, Blocklist and Allowlist) you need to add `$` to the end of your URL. This prevents anything following the endpoint being mocked as well. For example, adding `/mock` also means `/mock/somepath` can also be mocked. Using `/mock$` prevents `/somepath` being added and mocked to your endpoint.
{{< /note >}}
...
Adding a path to a Blocklist will force it to be blocked. This can be useful if you are versioning your API and are deprecating a resource. Instead of just making the path vanish you can block access to it.

Accessing a path which has been Blocklisted:
...
By default the Blocklist endpoint plugin is case-sensitive, so for example if `getuser` is Blocklisted, `getUser` and `GetUser` will not be Blocklisted. If you select the **Ignore Case** option from the Blocklist plugin settings, `getUser`, `GetUser` and `getuser` will all be Blocklisted in the above example.
...
![Deny ist options](/docs/img/2.10/blocklist.png)
...
In order for mocks to be enabled, the path must also be in a list. We recommend adding the path to an Allowlist. If this isn't done, then the mock will not be saved on an update.
{{< /note >}}


**API Blueprint**: If you have imported an API Blueprint definition, and selected the mocks option in the importer, then your whole API will be an Allowlist.
...
Adding a path to a Allowlist will cause the entire API to become Blocklisted. This means any non-specified routes will be blocked, and only those listed in the Endpoint Designer will be allowed through. This is great if you wish to have very select access rules for your services.

Accessing a path which has **not** been Allowlisted:
...
By default the Allowlist endpoint plugin is case-sensitive, so for example if `getuser` is Allowlisted, `getUser` and `GetUser` will not be Allowlisted. If you select the **Ignore Case** option from the Allowlist plugin settings, `getUser`, `GetUser` and `getuser` will all be Allowlisted in the above example.
...
![Allow list options](/docs/img/2.10/allowlist.png)
