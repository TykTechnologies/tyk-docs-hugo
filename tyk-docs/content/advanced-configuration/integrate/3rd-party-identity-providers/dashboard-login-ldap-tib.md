...
This guide assumes you already have a Tyk environment set up, with a Gateway and Dashboard. If you don't, please follow the [Tyk On-Premises getting started guide](/getting-started/installation/with-tyk-on-premises/).
...
* `OrgId`: The organisation id which the profile is connected to - make sure this is the correct id for your organisation (see the [Dashboard Admin API documentation](/tyk-apis/tyk-dashboard-admin-api/organisations/) for details on how to retrieve this)
...
When TIB is running you can use its REST API. See the [TIB REST API documentation](/advanced-configuration/integrate/3rd-party-identity-providers/tib-rest-api/) for a full description of each endpoint.
