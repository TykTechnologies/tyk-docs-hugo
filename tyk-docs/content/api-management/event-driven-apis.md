...
In this scenario, a user sends a message (e.g., asking for a joke) via a simple web interface, and receives a response generated by a backend service asynchronously.
...
*   **Flow:**
    ...
    3.  A backend service (our "Joker Service") is listening to the `chat` topic for incoming requests.
*   **Value Demonstrated:**
...
*   **Goal:** Deliver the asynchronous response (the joke) from the backend system back to the client in real-time.
...
*   **Tyk API Gateway**: The core API management platform. It serves as the entry point, handling initial request processing (like authentication and rate limiting) and routing requests.
*   **Tyk Streams Module**: An integrated extension within the Gateway specifically designed for asynchronous communication. It intercepts relevant requests and manages the streaming logic.
...
Think of the Tyk Gateway as the central dispatch for all API traffic. When traffic requires asynchronous handling (like pushing data to Kafka or subscribing to an MQTT topic), the integrated Tyk Streams module takes over, managing the interaction with the specific Event Broker and Upstream Service according to the API's configuration.
...
6.  **Analytics Capture**: Throughout the stream handling process, relevant metrics are captured by the `Stream Analytics` component.
...
If you manage your APIs via the Tyk Dashboard, you also need to enable Streams support within the Dashboard configuration (`tyk_analytics.conf`) to expose Streams-related UI elements and functionality.
...
Tyk Streams offers protocol mediation capabilities that allow you to expose async APIs using different protocols than the backend event broker. For example, you can expose a Kafka topic as a WebSocket API, convert MQTT messages to HTTP webhooks, or bridge between different async protocols (e.g., Kafka to MQTT).

</details> <details> <summary><b>What deployment considerations should I keep in mind for Tyk Streams?</b></summary>

Tyk Streams is embedded within the Tyk Gateway, so it scales with your existing Tyk infrastructure. No additional configuration or separate infrastructure is required.

</details> <details> <summary><b>Is Tyk Streams available in all Tyk editions?</b></summary>

Tyk Streams is a newer feature and available only in the `enterprise` edition. Currently, on Tyk Cloud, it's only available for hybrid customers. Check the latest documentation or contact Tyk support for specific availability in your edition.
