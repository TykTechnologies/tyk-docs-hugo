...
For example, given the base Tyk OAS API used [here]({{< ref "api-management/gateway-config-managing-oas#tutorial-3-update-tyk-oas-api-definition-with-an-updated-openapi-definition" >}}) we can enable the `allowList` middleware with this `PATCH` command:
