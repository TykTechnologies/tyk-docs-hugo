...
and provides a [Virtual Endpoint]({{< ref "api-management/traffic-transformation#virtual-endpoint-overview" >}}) style capability using the Go language, rather than JavaScript (as supported by the virtual endpoint middleware).
...
This plugin allows you to add custom logic to the processing flow for the specific endpoint without adding to the processing complexity of other endpoints.
It can [terminate the request]({{< ref "api-management/plugins/golang#terminating-the-request" >}}), if required,
and hence can provide a [Virtual Endpoint]({{< ref "api-management/traffic-transformation#virtual-endpoint-overview" >}}) style capability using the Go language, rather than JavaScript (as supported by the virtual endpoint middleware).
