...
**Issue Description:**  
We have identified an issue affecting Tyk Gateway deployed as a data plane connecting to the Multi-Data Center Bridge (MDCB) control plane or Tyk Cloud. In the above-mentioned Gateway versions, a panic may occur when the gateway reconnects to the control plane after the control plane is restarted.
...
- **For users on versions 5.5.0, 5.3.5, and 5.0.13**:  
  Delay upgrading to the affected versions until the patch is available.
- **For users who have already upgraded to 5.6.0, 5.3.6, or 5.0.14 and are experiencing a panic in the gateway**:  
  Restarting the gateway process will restore it to a healthy state.  
  In Kubernetes, the Gateway should automatically restart and recover.

We appreciate your patience as we work to resolve this. Stay tuned for the upcoming patch.
...
**Issue Description:**  
We have identified an issue affecting Tyk Gateway deployed as a data plane connecting to the Multi-Data Center Bridge (MDCB) control plane or Tyk Cloud.  
In the above-mentioned Gateway versions, a panic may occur when the Gateway reconnects to the control plane after it is restarted.

Our engineering team is actively working on a fix. A patch (versions 5.6.1, 5.3.7, and 5.0.15) will be released soon.
...
- **For users who have already upgraded to 5.6.0, 5.3.6, or 5.0.14 and are experiencing a panic in the Gateway**:  
  Restarting the Gateway process will restore it to a healthy state.  
  If you're running in a *Kubernetes* environment, the Gateway should automatically restart and recover.

We appreciate your understanding and patience as we work to resolve this.  
Please stay tuned for the upcoming patch release that will address the issue.
