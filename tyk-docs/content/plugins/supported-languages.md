...
*   [Rich Plugins]({{< ref "plugins/supported-languages/rich-plugins" >}}) includes [Python, Lua, gRPC] - With gRPC, you can write plugins in Java, .NET, C++ / C#, PHP, [and all other supported languages](https://grpc.io/docs/languages/).
...
* Used to stop the request and return a response / error - using [ReturnOverrides]({{< ref "plugins/plugin-types/request-plugins#return-overrides-returnoverrides" >}})
...
[More reading on the hook types in rich plugins]({{< ref "plugins/supported-languages/rich-plugins/rich-plugins-work#coprocess-dispatcher---hooks" >}}) and explanation with common use case for each [hook type]({{<ref "plugins/plugin-types/plugintypes">}}) 
