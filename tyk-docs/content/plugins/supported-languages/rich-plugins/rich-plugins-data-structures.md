...
`monitor`
Defines the quota monitor percentage threshold limits that trigger webhook notification when reached. See [Monitor](#monitor) for further details and an example.
...
Added to a [session](#sessionstate-session_stateproto) when monitor quota thresholds are defined within the Tyk key. This message contains the quota percentage threshold limits, defined in descending order, that trigger webhook notification when reached.
...
`trigger_limits`
List of trigger limits defined in descending order. Each trigger limit represents the percentage of the quota that must be reached in order for the trigger to be fired.
