...
* The `custom_middleware` block contains the middleware settings like the plugin driver we want to use (`driver`) and the hooks that our plugin will expose. You use the `auth_check` for this tutorial. For other hooks see [here](/plugins/rich-plugins/rich-plugins-work/#coprocess-dispatcher---hooks).
...
You import decorators from the Tyk module as this gives you the `Hook` decorator, and you import [Tyk Python API helpers](/plugins/rich-plugins/python/tyk-python-api-methods/)
