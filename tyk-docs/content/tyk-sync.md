...
Tyk Sync works with Git and any CI/CD tool to ensure that only versioned changes are loaded into your Tyk Environment.  After an API developer has configured and tested the APIs locally, he can use "tyk-sync dump" to convert the APIs to transportable format. Following GitOps standard practises, the API developer should create a Pull-Request on Git to get peer review and version the final API configurations. To sync configurations from Git to a target Tyk installation, use "tyk-sync sync", "tyk-sync publish" or "tyk-sync update" in the deployment pipeline.
