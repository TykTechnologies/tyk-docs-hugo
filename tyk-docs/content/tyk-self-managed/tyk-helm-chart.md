...
To ensure that your Gateway pods will not scale beyond your license allowance, set the Gateway's resource kind to *Deployment*
and the replica count to your license node limit. By default, the chart is configured to work with single node license: `gateway.kind=Deployment` and `gateway.replicaCount=1` in `values.yaml`. You can update the default settings using following options: `--set gateway.kind=Deployment --set gateway.replicaCount=1` in your `values.yaml` file or in the Helm install command.
