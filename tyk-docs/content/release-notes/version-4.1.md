...
Tyk has always had a proprietary specification for defining APIs. From Tyk v4.1 we now support defining APIs using the Open API Specification (OAS) as well, which can offer significant time and complexity savings. [This is an early access capability](/frequently-asked-questions/using-early-access-features/).
...
This capability is available in both the open source and paid versions of Tyk. See our [High Level Concepts](/getting-started/key-concepts/high-level-concepts/) for more details, or jump to [OAS Getting Started documentation](/getting-started/using-oas-definitions/create-an-oas-api/).
