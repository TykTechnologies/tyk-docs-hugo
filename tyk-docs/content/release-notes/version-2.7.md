...
Instead of setting permissions per user, you can now [create a user group](/basic-config-and-security/security/dashboard/create-user-groups/), and assign it to multiple users. It works for Single Sign-On too, just specify group ID during [SSO API](/tyk-apis/tyk-dashboard-admin-api/sso/) flow.
...
From an API standpoint, user groups can be managed by [new Dashboard API](/tyk-apis/tyk-dashboard-api/user-groups/). The User object now has a new `group_id` field, and if it is specified, all permissions will be inherited from the specified group. [SSO API](/tyk-apis/tyk-dashboard-admin-api/sso/) has been updated to include `group_id` field as well.

### Added SMTP support
Now you can configure the Dashboard to send transactional emails using your SMTP provider. See [Outbound Email Configuration](/tyk-configuration-reference/outbound-email-configuration/) for details.

## <a name="upgrade"></a>Upgrading all new Components

For details on upgrading all Tyk versions, see [Upgrading Tyk](/upgrading-tyk/).
