...
While SQL support for Tyk products does not depend on specific database features, with this release, we will provide official support for [PostgreSQL DB for production purposes]({{< ref "/content/migration-to-tyk#database-management.md" >}}), and SQLite for development and PoC environments. Note that SQL support is available for self-managed setups only.
...
[MongoDB to SQL migration docs]({{< ref "/content/migration-to-tyk#database-management" >}})
...
If you want switch from MongoDB to SQL, you can [use our migration tool]({{< ref "/content/migration-to-tyk#database-management" >}}), but keep in mind that it does not yet support the migration of your analytics data.
