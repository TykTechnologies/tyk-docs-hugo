...
In the above example an incoming request of `/basic/456` would be matched by the `match_pattern` rule `/basic/(.*)` for `GET` requests specified in the `method` field. The `456` part of the URL will be captured and replaces `{id}` in the `path` field. The request will then be redirected to URL `tyk://ZGVmYXVsdC9wcm94eS1hcGk/proxy/456` obtained from the `rewrite_to` field generated by Tyk Operator. The context variable `$1` will be substituted with the `456` value.

Furthemore, the `rewrite_to_internal` object targets an `ApiDefinition` resource named `proxy-api` in the `default` namespace. The `path` within the `rewrite_to_internal` object is configured as `proxy/$1`. Tyk Operator will use this to automatically generate a URL redirect the request to the `proxy-api` API in the `default` namespace as shown below:
