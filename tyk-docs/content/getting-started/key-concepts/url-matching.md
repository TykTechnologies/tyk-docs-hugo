...
- [Allowlist]({{< ref "advanced-configuration/transform-traffic/endpoint-designer#allowlist" >}})
- [Blocklist]({{< ref "advanced-configuration/transform-traffic/endpoint-designer#blocklist" >}})
...
If [URL path versioning]({{< ref "getting-started/key-concepts/versioning#first-url-element" >}}) is in use for an API, Tyk will perform a match of the first fragment after the *listen path* to identify which version of the API should be invoked.
