...
For more information on mock responses, see the dedicated [page]({{< ref "api-management/traffic-transformation#mock-response" >}}).
...
The process for importing Tyk Classic API definition or SOAP WSDL definition is explained [here]({{< ref "api-management/gateway-config-managing-classic#import-an-api" >}}). The import function will now accept an existing Tyk OAS API definition, the process is the same as for a Tyk Classic API definition.
