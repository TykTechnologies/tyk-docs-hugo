...
| Type         | None                                                                           |
| Body         | Tyk OAS API Definition                                                         |
| Query Param. | Options: <br>- `base_api_id`: The base API ID which the new version will be linked to.<br>- `base_api_version_name`: The version name of the base API while creating the first version. This doesn't have to be sent for the next versions but if it is set, it will override base API version name.<br>- `new_version_name`: The version name of the created version.<br>- `set_default`: If true, the new version is set as default version.|
