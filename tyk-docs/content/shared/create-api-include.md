...
- **Authentication mode**: This is the security method to use with your API.  For this tutorial, set it to `Open (Keyless)`. See [Authentication and Authorization](/basic-config-and-security/security/authentication-authorization/) for more details on securing your API.
...
- **Enable client certificate**: Select this to use Mutual TLS. See [Mutual TLS](/basic-config-and-security/security/tls-and-ssl/mutual-tls/) for details on implementing mutual TLS.
